...
try:
		...
		testGroup = -1
#		error = True	
#if error: raise ValueError('./srange testGroup,  testGroup must be >0')
...
if testGroup & 1:
	...
	test('')								# empty string
	test(3.65)								# illegal type (should be str)
	test('3.65')							# illegal value (should be integer)
	test('-3-3, 5-11')						# negative values
	test([3, 5, 9, 10, 11, 12])				# a list as input
	test([3.141, 6.282, 21.163])			# illegal, list of non-integers
	try:
		import numpy
